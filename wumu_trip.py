...
class WumuTriptychAtelier:
        ...
        return {
            "required": {
                "char_name": ("STRING", {"default": "角色名",
                    "tooltip": "角色中文名（文件命名用，与定妆照工坊保持一致）"}),
                "unet_name": (unets, {"default": unets[0] if unets else "flux-2-klein-9b-kv-fp8.safetensors",
                    "tooltip": "Klein 底模（9B，快速，自动过滤 klein）"}),
                "clip_name": (clips, {"default": clips[0] if clips else "qwen_3_8b_fp8mixed.safetensors",
                    "tooltip": "Qwen3 编码器（中文提示词原生）"}),
                "vae_name": (vaes, {"default": vaes[0] if vaes else "full_encoder_small_decoder.safetensors",
                    "tooltip": "FLUX.2 VAE（128 通道）"}),
                "steps": ("INT", {"default": 4, "min": 1, "max": 50,
                    "tooltip": "采样步数，4 = Klein 标准值"}),
                "seed": ("INT", {"default": 0, "min": 0, "max": 2**32 - 1,
                    "tooltip": "种子：每槽 +100，每条链再 +1/+10/+20/+30"}),
                "canvas_w": ("INT", {"default": 1920, "min": 512, "max": 4096, "tooltip": "四联图画布宽"}),
                "canvas_h": ("INT", {"default": 1080, "min": 512, "max": 4096, "tooltip": "四联图画布高"}),
                "front_h": ("INT", {"default": 960, "min": 256, "max": 4096,
                    "tooltip": "证件照目标高度（大图）\n四图自动适配后实际约 745px"}),
                "side_h": ("INT", {"default": 820, "min": 256, "max": 4096,
                    "tooltip": "正面/侧面/背面三张全身照目标高度\n四图自动适配后实际约 637px"}),
                "slot1_name": ("STRING", {"default": "睡衣", "tooltip": "造型1中文名（须与定妆照工坊同名）"}),
                "slot2_name": ("STRING", {"default": "造型2"}),
                "slot3_name": ("STRING", {"default": "造型3"}),
                "slot4_name": ("STRING", {"default": "造型4"}),
                "slot5_name": ("STRING", {"default": "造型5"}),
                "auto_save": ("BOOLEAN", {"default": True, "tooltip": "自动保存到 资产\\角色\\"}),
                "save_views": ("BOOLEAN", {"default": True,
                    "tooltip": "另存单视角图（证件照/正面图/侧面图/背面图）\n导演台引用和引擎 refs 需要这些文件，建议开"}),                "save_dir": ("STRING", {"default": r"F:\AI\MINIMAXH3"}),
            },
            "optional": {
                "slot1_image": ("IMAGE", {"tooltip": "造型1定妆照：接定妆照工坊输出 或 LoadImage"}),
                "slot2_image": ("IMAGE", {"tooltip": "造型2定妆照"}),
                "slot3_image": ("IMAGE", {"tooltip": "造型3定妆照"}),
                "slot4_image": ("IMAGE", {"tooltip": "造型4定妆照"}),
                "slot5_image": ("IMAGE", {"tooltip": "造型5定妆照"}),
            },
        }
    ...
    FUNCTION = "run"
    CATEGORY = "wumu"
    DESCRIPTION = "Wumu 四联图工坊（Klein）：造型定妆照 → 证件照+正/侧/后四链（纯白背景）→ 1920×1080 合成四联图，一个节点出全部角色参考图"
    ...
    def _generate_slot(self, model, clip, vae, img, seed, steps,
        ...
        base = "生成图中角色"
        p_id = f"{base}的证件照式正面半身照（头肩特写、五官清晰、表情自然、纯白背景），保持人物一致性。"
        p_front = f"{base}全身的正视图，纯白色摄影棚背景，保持人物一致性。"
        p_side = f"{base}的侧视图，纯白色摄影棚背景。"
        p_back = f"{base}的后视图，纯白色摄影棚背景，保持人物一致性。"

        print(f"[Wumu四联工坊] 📸 证件照 (1024×1024)")
        id_img = tensor_to_pil(vae.decode(
            self._sample(model, self._encode(clip, p_id, [ref_latent]), 1024, 1024, steps, seed + 1)))

        print(f"[Wumu四联工坊] 🧍 全身正视图 (1024×1920)")
        ...
        print(f"[Wumu四联工坊] 🔄 侧视图（参考正视输出+原图）")
    ...
    def run(self, char_name, unet_name, clip_name, vae_name, steps, seed,
        ...
        print(f"[Wumu四联工坊] 🚀 角色={char_name} 槽数={len(active)} 模型={unet_name} 步数={steps}")
        ...
        for i, (im, nm) in enumerate(slots):
            if not valid(im):
                results.append(None)
                continue
            print(f"[Wumu四联工坊] --- 槽{i + 1}: {nm} ---")
            slot_seed = seed + i * 100
            id_img, front_img, side_img, back_img, tri = self._generate_slot(
                model, clip, vae, im, slot_seed, steps, canvas_w, canvas_h, front_h, side_h)
            if auto_save:
                d = os.path.join(save_dir, "资产", "角色")
                os.makedirs(d, exist_ok=True)
                if save_views:
                    id_img.save(os.path.join(d, f"{char_name}_{nm}_证件照.png"))
                    front_img.save(os.path.join(d, f"{char_name}_{nm}_正面图.png"))
                    side_img.save(os.path.join(d, f"{char_name}_{nm}_侧面图.png"))
                    back_img.save(os.path.join(d, f"{char_name}_{nm}_背面图.png"))
                tri.save(os.path.join(d, f"{char_name}_{nm}_四联图.png"))
                print(f"[Wumu四联工坊] 💾 已保存 角色={char_name} 造型={nm}（四联图{'+四视角' if save_views else ''}）")
        ...
        while len(out) < 5:
            out.append(blank)
        info = f"角色={char_name} | 四联图×{len(active)} | Klein={unet_name} | {steps}步"
